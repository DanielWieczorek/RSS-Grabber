...
public final class NetworkInputBuilder {
    ...
    public static double[] buildInputArray(IStateHistoryHolder state, Account account) {
        ...
        double[] result = new double[20 * parts.size() + 2];

        // Check if at least 2 parts
        for (int i = 0; i < parts.size(); i++) {
            StateEdgePart currentPart = parts.get(i);
            // result[21 * i + 0] = currentPart.getSentiment() != null ?
            // currentPart.getSentiment().getPredictedDelta()
            // : 0.0;
            ...
            for (int j = 0; j < record.size(); j++) {
                result[20 * i + j * 5 + 0] = Double.isNaN(record.get(j).getValue1min()) ? 0.0
                        : record.get(j).getValue1min();
                result[20 * i + j * 5 + 1] = Double.isNaN(record.get(j).getValue5min()) ? 0.0
                        : record.get(j).getValue5min();
                result[20 * i + j * 5 + 2] = Double.isNaN(record.get(j).getValue15min()) ? 0.0
                        : record.get(j).getValue15min();
                result[20 * i + j * 5 + 3] = Double.isNaN(record.get(j).getValue30min()) ? 0.0
                        : record.get(j).getValue30min();
                result[20 * i + j * 5 + 4] = Double.isNaN(record.get(j).getValue60min()) ? 0.0
                        : record.get(j).getValue60min();
            }

        }

        result[20 * parts.size() + 0] = account.getBtc() > 0.0 ? 1.0 : 0.0;
        result[20 * parts.size() + 1] = account.getEur() > 0.0 ? 1.0 : 0.0;
